...
> **進め方**
> まず **お手本(`app/`)を読んで理解する**こと。`models.py` → `schemas.py` → `database.py` → `crud.py` → `routers/todos.py` → `main.py` の順がおすすめ。
...
- エンドポイント（すべて `/todos`）:
  ...
  - `DELETE /todos/{id}` … 削除
- 主なファイル: `app/models.py` / `app/schemas.py` / `app/database.py` / `app/crud.py` / `app/routers/todos.py` / `app/main.py`
  - **役割分担を意識する**:
    - `app/crud.py` … DB の読み書きそのもの（`get_todos` / `get_todo` / `create_todo` / `update_todo` / `delete_todo`）。`Session` を引数で受け取り、HTTPException は投げない。
    - `app/routers/todos.py` … エンドポイント（URL・ステータスコード・404 判定）。crud の関数を呼ぶ**薄い層**。
    - `app/main.py` … アプリの組み立てだけ（lifespan・`/` でフロント配信・`include_router`）。**ロジックは置かない**。
  - お手本では「DB 操作 = crud.py」「HTTP の入口 = routers/todos.py」に分かれている。課題でも**この 2 つを両方直す**ことになる場面が多い。
- パッケージ管理は **uv**
  - テスト実行: `uv run pytest`
...
- `app/crud.py`: `create_todo` で、受け取った `TodoCreate` の `assignee` を `Todo` に渡して保存する。
  - 保存処理の実体はここ。`Todo(title=..., done=..., assignee=...)` のように 1 項目足す。
- `app/routers/todos.py`: 通常は変更不要（`crud.create_todo` を呼ぶだけの薄い層なので、入出力の形は schemas が面倒を見る）。
- **DBの作り直し**を行う（下のヒント必読）。
...
- 保存処理は `routers/todos.py` ではなく `crud.py` にある。「DB に書く処理は crud」「HTTP の入口は routers」という分担をお手本のコメントで確認しておく。
...
### やること
- `app/routers/todos.py`: `list_todos` に「担当者名」のクエリパラメータ（任意）を追加し、`crud.get_todos` に渡す。
  - クエリパラメータの受け取り（URL の話）は HTTP の入口＝routers の担当。
- `app/crud.py`: `get_todos` が「担当者名（任意）」を受け取り、渡されたときだけ `where` で絞るようにする。
  - パラメータが渡されたときだけ、その担当者のものに絞る。
  - 渡されなければ全件返す（今の挙動を壊さない）。実際の DB クエリ（`select`/`where`）は crud の担当。

### ヒント
- FastAPI のクエリパラメータ: `routers/todos.py` の `list_todos` の引数にデフォルト値付きで書くと、URL の `?key=value` を受け取れる。
  - 任意にしたいので `str | None = None`（Optional）の形。
- SQLModel の絞り込みは `crud.py` の `get_todos` の中で `select(Todo).where(...)` を使う。
  - 「パラメータがあるときだけ `where` を足す」という分岐を書く。
  - `select` した文に対して、条件があるときだけ `.where(Todo.assignee == ...)` を追加する発想。
- 「URL からの受け取り（routers）」と「DB の絞り込み（crud）」を分けて考えると整理しやすい。
...
- `app/crud.py`: 作成まわりを `user_id` ベースに直す（`create_todo` で `user_id` を保存）。`User` を作る／引く DB 関数を足してもよい。
- `app/routers/todos.py`: `User` を作るエンドポイント（例: `POST /users`）を足すなら、ここに新しいルーターか route を用意し、crud の関数を呼ぶ。
  - 別ファイル `app/routers/users.py` を新設して `main.py` で `include_router` する形だと、お手本の分け方に沿っていてキレイ。
- `app/main.py`: 新しいルーターを足したら `include_router` で登録する（ロジックは置かない）。
...
- `app/crud.py`:
  - `create_todo` で `due_date` を受け取って保存する。
  - 期限切れだけを引く DB 関数（例: `get_overdue_todos`）を足す（`select`/`where` の実体はここ）。
- `app/routers/todos.py`:
  - 新しいエンドポイント `GET /todos/overdue` を追加し、`crud.get_overdue_todos` を呼んで返す。
- `app/main.py`: 変更不要（既存の `todos.router` に route を足すだけなので）。
- `todo.db` を作り直す。

### ヒント
- nullable な日時: 型を `datetime | None`、デフォルト `None` にする。
- 「期限切れ」= `due_date` が**現在時刻より前**で、かつ `due_date` がある（`None` ではない）もの。
- 現在時刻は `datetime.now()`。これと `Todo.due_date` を `where` で比較する（`crud.py` の中で）。
  - `select(Todo).where(Todo.due_date < datetime.now())` のような発想。`None` を除く条件も忘れずに。
- **ルート定義の順番に注意**: `routers/todos.py` の中で、`GET /todos/{todo_id}` より**前**に `GET /todos/overdue` を定義する。
  - 後ろに書くと `overdue` が `{todo_id}` に吸われて `int` 変換エラーになりがち。なぜそうなるか考えてみる。
  - 補足: ルーターは `prefix="/todos"` 付きなので、ファイル内ではパスを `/overdue` と書けば `/todos/overdue` になる。
